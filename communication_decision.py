"""
通信干扰实时决策模拟程序 — 流式有状态版本（含每步数据保存及 MQTT 实时发送）
"""
...
try:
    ...
    JAMMING_MODEL_AVAILABLE = False
    logger.warning("未找到 jamming_success_rate 模块，将使用简化指数模型")
...
def find_distance_for_success_rate(jam_type: str, target_success_rate: float = 0.7):
    if JAMMING_MODEL_AVAILABLE:
        jammer = JammerParams(tx_power=20, jam_power=40, antenna_gain=12, frequency=2.4e9,
                              bandwidth=200e6, modulation='QPSK')
        target = TargetParams(tx_power=10, antenna_gain=15, frequency=2.4e9, bandwidth=10e6,
                               modulation='QPSK', range=1000)
        low, high = 0.1, 100.0
        for _ in range(50):
            mid = (low + high) / 2
            rate = jamming_success_rate(jammer, target, mid, jam_type)
            if rate > target_success_rate: low = mid
            else: high = mid
        return low * 1000
    else:
        if jam_type == 'suppressive':
            dist_km = -5 * math.log(target_success_rate)
        else:
            dist_km = -20 * math.log(target_success_rate)
        return dist_km * 1000
...
class Jammer:
    def __init__(self, idx, uuid, x, y, alt, mode, show_name):
        self.id = idx
        self.uuid = uuid
        self.x = x
        self.y = y
...
class Target:
    def __init__(self, idx, name, threat_value, threat_grade, scene_radius, comm_center_freq_hz):
        self.id = idx
        self.name = name
        ...
        self.comm_center_freq_hz = comm_center_freq_hz
...
class CommunicationJammerSimulation:
    comm_strategies = [
        {"mode": "navigation_deception", "params": {"center_freq_mhz": 1580.0}, "jam_type": "deceptive"},
        {"mode": "drive_away",          "params": {"center_freq_mhz": 5700.0}, "jam_type": "deceptive"},
        {"mode": "navigation_jamming",  "params": {"center_freq_mhz": 2450.0}, "jam_type": "deceptive"},
        {"mode": "navigation_defense",  "params": {"center_freq_mhz": 5800.0}, "jam_type": "deceptive"},
        {"mode": "no_fly_landing",      "params": {"center_freq_mhz": 2450.0}, "jam_type": "deceptive"},
        {"mode": "frequency_hopping",   "params": {"center_freq_mhz": 2450.0}, "jam_type": "suppressive"},
        {"mode": "barrage",             "params": {"center_freq_mhz": 5780.0}, "jam_type": "suppressive"},
        {"mode": "spot",                "params": {"center_freq_mhz": 5800.0}, "jam_type": "suppressive"},
    ]
    ...
    def create_stream_simulator(cls, project_name: str, deployment: str = 'optimized', silent: bool = True,
        ...
        if not os.path.exists(jammer_file):
            raise FileNotFoundError(f"未找到干扰机文件: {jammer_file}")
        ...
        sim.target_comm_params = {}
        for tgt_info in sim.comm_data.get('targets', []):
            idx = tgt_info.get('baseInfo', {}).get('index')
            if idx is not None:
                comm_info = tgt_info.get('modelInfo', {}).get('commSignalInfo', {})
                freq_hz = parse_freq_to_hz(comm_info.get('carrierFreq', '0 Hz'))
                sim.target_comm_params[idx] = freq_hz

        sim.results_dir = os.path.join(config_dir, "data", project_name, "communication_data",
        ...
        try:
            ...
            logger.info(f"[通信-{deployment}] 数据目录已创建: {sim.data_dir}")
        except Exception as e:
            logger.error(f"[通信-{deployment}] 创建数据目录失败: {e}")

        logger.info(f"[通信] 流式模拟器创建成功，部署类型: {deployment}，干扰机数量: {sim.num_jammers}，有效半径: 压制{sim.radius_suppressive:.0f}m 欺骗{sim.radius_deceptive:.0f}m")
        return sim

    def _setup_coordinate_system(self):
        scene = self.comm_data['scene']
    ...
    def _load_jammers_and_guardpoints(self):
        ...
        for gp in self.comm_data['guardpoints']:
        ...
        if self.deployment == 'random':
            jammer_list_key = 'originalJammers' if 'originalJammers' in self.comm_data else 'originaljammers'
        else:
            jammer_list_key = 'optimizedJammers' if 'optimizedJammers' in self.comm_data else 'optimizedjammers'

        jammer_data = self.comm_data.get(jammer_list_key, [])
        if not jammer_data:
            raise ValueError(f"未找到干扰机列表: {jammer_list_key}")

        default_mode = self.comm_strategies[0]['mode'] if self.comm_strategies else "communication"
        self.jammers = []
        for i, jam in enumerate(jammer_data):
            ...
            x, y = self.converter.lonlat_to_xy(lon, lat)
            show_name = jam.get('showName', f"通信干扰机_{jam['uuid']}")
            jammer = Jammer(i, jam['uuid'], x, y, alt, default_mode, show_name)
            self.jammers.append(jammer)

        logger.info(f"[通信-{self.deployment}] 加载 {len(self.jammers)} 个干扰机，示例坐标: ({self.jammers[0].x:.0f}, {self.jammers[0].y:.0f})")
    ...
    def _ensure_target_exists(self, tgt_info: dict, t_rel: float):
        ...
        if tid not in self.target_dict:
            ...
            comm_freq = self.target_comm_params.get(tid, 1e9)
            target = Target(tid, name, threat_val, threat_grade, self.radius_m, comm_freq)
            self.target_dict[tid] = target
            self.targets.append(target)
            logger.info(f"[通信-{self.deployment}] 新目标 {tid}，威胁值 {threat_val:.2f}，通信频率 {comm_freq/1e6:.1f}MHz")
        ...
        logger.debug(f"[通信-{self.deployment}] 目标{tid} 位置更新: t={t_rel:.1f}s, xy=({x:.0f},{y:.0f})")
        return self.target_dict[tid]

    def update_with_frame(self, frame: dict, t_rel: float):
        """接收新一帧目标数据，推进模拟"""
        targets_in_frame = frame.get('target_info', [])
        logger.info(f"[通信-{self.deployment}] 收到帧，t_rel={t_rel:.2f}，目标数={len(targets_in_frame)}")
        if targets_in_frame:
            sample = targets_in_frame[0]
            loc = sample.get('location', [])
            if len(loc) >= 2:
                logger.info(f"[通信-{self.deployment}] 示例目标: id={sample.get('id')}, lon={loc[0]:.6f}, lat={loc[1]:.6f}")
        ...
        if self.sim_start_datetime is None:
            ...
            if frame_dt:
                ...
                logger.info(f"[通信-{self.deployment}] 模拟起始时间: {self.sim_start_datetime.strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]}")
            else:
                logger.warning(f"[通信-{self.deployment}] 无效时间戳，使用系统当前时间")
        ...
        if t_rel <= self._current_time:
            logger.debug(f"[通信-{self.deployment}] 时间未推进 (当前={self._current_time:.2f})，跳过")
            return
        ...
        logger.info(f"[通信-{self.deployment}] 推进完成，步数={steps}，当前时间={self._current_time:.2f}，目标数={len(self.targets)}，激活干扰机={active_count}")
    ...
    def _save_data(self, t: float):
        ...
        try:
            ...
            logger.debug(f"[通信-{self.deployment}] 保存步数据: {filepath}")
        except Exception as e:
            logger.error(f"[通信-{self.deployment}] 保存数据失败 {filepath}: {e}")

        # MQTT 实时发送
        if self.mqtt_callback is not None:
            try:
                self.mqtt_callback(data)
                logger.debug(f"[通信-{self.deployment}] MQTT 步数据已发送, t={t:.1f}")
            except Exception as e:
                logger.error(f"[通信-{self.deployment}] MQTT 发送步数据失败: {e}", exc_info=True)
    ...
    def finalize(self) -> Dict[str, Any]:
        ...
        logger.info(f"[通信-{self.deployment}] 最终指标: 总时间={total_time:.1f}s, 平均干扰时长={overall['jam_duration']:.2f}s, 总有效长度={overall['effective_jamming_length']:.2f}m")
